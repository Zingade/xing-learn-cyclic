...
import {currentSortYear, MONTH_COLUMNS} from '../../Utils/CommonConstants'
    ...
    {(loading) ? (
        ...
        <div className={classes.table}>
          <div className={classes.autoScroll}>
            ...
            >
            {findGrandTotal(studentList, grandTotal)}
              <div className="row heading">
                ...
                {MONTH_COLUMNS.map((month,count) => (
                  <HeaderCell
                    key={count}
                    value={capitalizeCustom(STUDENT_MAPPING[month].displayName + ((count >= 8)? parseInt(currentSortYear.value) + 1: currentSortYear.value))}
                  />
                  ))}
              </div>
