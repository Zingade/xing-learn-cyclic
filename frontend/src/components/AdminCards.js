...
import { BATCH_BUMBER_VALUE, LAST_MONTH_AMOUNT_VALUE, TOTAL_AMOUNT_VALUE, summaryFees, THIS_MONTH_AMOUNT_VALUE, currentSortYear } from './Utils/CommonConstants';
...
function findGrandTotal(users)  {
    const currentMonth = (((new Date()).getMonth()+8)%12)
...
function FeesSummaryCard (props) {
    ...
    return (<>
        <Grid container spacing={1} className={classes.summaryContainer}>
            <SnapBoard text={"Total"} number={summaryFees.total.Overall} color={"#ff073a"}/>
            <SnapBoard text={"Cash"} number={summaryFees.total.Cash} color={"#007bff"}/>
            <SnapBoard text={"GPay"} number={summaryFees.total.GPay} color={"#28a745"}/>
        </Grid>
          <Grid container spacing={1} className={classes.summaryContainer}>
            <SnapBoard text={"This month"} number={summaryFees.delta.Overall} color={"#ff073a99"}/>
            <SnapBoard text={"Cash"} number={summaryFees.delta.Cash} color={"#007bff99"}/>
            <SnapBoard text={"GPay"} number={summaryFees.delta.GPay} color={"#28a74599"}/>
        </Grid>
    </>
    );
}
...
function SnapBoard (props) {
  const classes = useStyles()
  const {id, text, number, name, onClickHandle,color} = props;

  return (<>
      <Grid item className={classes.snapBoardInside}>
        <Typography id={id} name={name} text={text} style={{fontSize:"25px", color:color}} onClick={onClickHandle}>
          {number}
        </Typography>
...
function StudCard(props) {
  ...
  const currentMonthFees = student.fees[(currentMonth+7)%12]; 
  const lastMonthFees = student.fees[(currentMonth+7-1)%12];
  ...
  const updateDatabase = (values) => {
    const index = (values.lastMMonth) ? (currentMonth+7-1)%12 : (currentMonth+7)%12 ; 
  ...
  return (
    <Card sx={{ maxWidth: 165 }} style={{backgroundColor:"#edeeef"}}>
      ...
      <Grid container justifyContent="center">
        <SnapBoard id={THIS_MONTH_AMOUNT_VALUE} name={student.name} text={"This month"} number={currentMonthFees.amount} onClickHandle={onClickHandle} color={(currentMonthFees.payMethod === "1")?"#28a74599":"#007bff99"}/>
        <SnapBoard id={LAST_MONTH_AMOUNT_VALUE} name={student.name} text={"Last month"} number={lastMonthFees.amount}  onClickHandle={onClickHandle} color={(lastMonthFees.payMethod === "1")?"#28a74599":"#007bff99"}/>
      </Grid>
      <Grid container justifyContent="center">
        <SnapBoard id={TOTAL_AMOUNT_VALUE} text={"Total"} number={totalFees} color={"#ff073a99"}/>
...
function StudentsCard (props) {
    ...
    var todaysDate = new Date();
    const currentMonth = todaysDate.getMonth()+1;
    const currentMonthFees = (currentMonth+7)%12; 
    const lastMonthFees = (currentMonth+7-1)%12;
    ...
    return (
        <>
            <Grid container spacing={1} className={classes.maincard}>
                <SortYear refreshPage={refreshPage}/>
                <FeesSummaryCard/>
